...
Shader "Unlit/HiShader"
{
    ...
    Properties
    {
        ...
        _Vector ("Example vector", Vector) = (.25, .5, .5, 1)
        // Inspector会显示拾色器拾取色彩RGBA值
        _Color("色彩", Color) = (.25, .5, .5, 1)
        // ————————————————————————————————————————————————
    ...
    SubShader
    {
        ...
        */
        ...
        // RenderType: 用来区别这个Shader要渲染的对象是属于什么类别的。设置渲染类型 用一种称为着色器替换的技术在运行时交换子着色器,用来区别这个Shader要渲染的对象是属于什么类别的
        // 这里表示非透明物体渲染
